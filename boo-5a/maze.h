...
public:
   ...
   void printPath(Graph::vertex_descriptor end,
                        ...
                        Graph g);  // Prints path contained in the stack
...
void maze::mapMazeToGraph(Graph &g) {
	...
	for (int col = 0; col < cols; col++) {
            ...
            if(row - 1 > 0) {
                if(value[row - 1][col])  {  // If above cell is filled
                    pair<Graph::edge_descriptor,bool> edge =
                    add_edge(graphCell[row][col], graphCell[row - 1][col], g);
                    g[edge.first].weight = 0;
                }
            ...
            if(col-1 > 0) {
                if(value[row][col - 1]) {  // If left cell is filled
                    pair<Graph::edge_descriptor,bool> edge =
                    add_edge(graphCell[row][col], graphCell[row][col - 1], g);
                    g[edge.first].weight = 0;
                }
...
void maze::printPath(Graph::vertex_descriptor end,
    ...
    // Set end row numbers
	endRow = row = g[end].cell.first;
	endCol = row = g[end].cell.second;

...
// Calls heap sort function from heap class
void wordList::sortHeap(const int& size) {
	heap<string> wordHeap;
	wordHeap.initializeMaxHeap(words);
	wordHeap.heapSort(size);
}
...
bool wordList::inList(const string& key) {

	// Get index of word in dictionary, if not found value is -1
	int foundWord = binarySearch(key);

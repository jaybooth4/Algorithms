...
bool board::assignPos(int &numRec)
{
...
// Check and set any cells with only 1 solution
	...
	while(oneSols) {
		oneSols = false;
		for (int ind = 1; ind <= 81; ind++)
		{
			row = ((ind - 1)/numRows) + 1;
			col = ((ind - 1)%numCols) + 1;
			if (getCell(row, col) == 0) {
				singleSol = numberSol(ind, 1);
				if(singleSol != -1) {
					oneSols = true;
					indexChanged.push_back(ind);
	...
	for (int i = 2; i <= 9; i++) {
		...
		while (index <= maxIndex) {
			...
			col = ((index - 1)%numCols) + 1;
			if (getCell(row, col) == 0) {
					if (numberSol(index, i) != -1) {
						for(int num = 1; num <= 9; num++) {
								if(!isConflict(row, col, num)) {
									setCell(row, col, num);
									indexChanged.push_back(index);
									solved = assignPos(numRec);  // Recursion!

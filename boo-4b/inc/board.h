...
const int numRows = boardSize;  // Used in calculation of row
const int numCols = boardSize;  // Number of columns
const int maxIndex = boardSize * boardSize;  // Maximum index in the board
...
class board
{
   public:
      ...
      void clearCell(int, int);  // Clears the cell and updates conflict values
      ...
      int numberSol(int, int); // Solves for the given open number at the index
      // Clears changes made on the recursive call
      void clearChanges(vector<int>&);
      // Recursive function to find the solution to the board
      bool assignNums(int&);
